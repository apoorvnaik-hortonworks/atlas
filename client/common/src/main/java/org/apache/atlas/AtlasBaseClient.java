...
public abstract class AtlasBaseClient {
    ...
    public boolean isServerReady() throws AtlasServiceException {
        API         versionAPI = getVersionAPI();
        WebResource resource   = getResource(versionAPI.getPath());
        try {
            callAPIWithResource(versionAPI, resource, null, JSONObject.class);
            return true;
    ...
    public String getAdminStatus() throws AtlasServiceException {
        ...
        API         statusAPI = getStatusAPI();
        WebResource resource  = getResource(service, statusAPI.getPath());
        JSONObject  response  = callAPIWithResource(statusAPI, resource, null, JSONObject.class);
        try {
    ...
    public AtlasMetrics getAtlasMetrics() throws AtlasServiceException {
        return callAPI(getMetricsAPI(), AtlasMetrics.class, null);
    }
    ...
    private String selectActiveServerAddress(Client client, AtlasServerEnsemble serverEnsemble)
        ...
        if (activeServerAddress != null)
            ...
            throw new AtlasServiceException(getStatusAPI(), new RuntimeException("Could not find any active instance"));
    }
